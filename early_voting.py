...
def cache_google_place_ids(google_api_key, max_retries=3, delay=0.02):
    ...
    polling_locations = []
    browser = webdriver.Firefox(executable_path="./selenium_drivers/firefox/geckodriver")
    ...
    for i, county in progress:
        ...
        browser.get(f'{voting_locations_url}idTown={county["idTown"]}')
        time.sleep(1)
        elem = browser.find_element(webdriver.common.by.By.ID, "rawdata-tab")
        elem.click()
        time.sleep(1)
        data_elem = browser.find_element(webdriver.common.by.By.CLASS_NAME, "data")
        places = json.loads(data_elem.text)
